 ...
 * Request PCBs on PCBWay: https://www.pcbway.com/project/shareproject/Coviclock___Table_clock_calendar_showing_temperature_and_coronavirus_data_for_Italian_Regions.html?from=settorezero2020
 * Subscribe to PCBWay using my invite link: https://www.pcbway.com/setinvite.aspx?inviteid=355653&from=settorezero2020
 * 
 * LICENSE
...
// this value will be added to the temperature reading
float TempAdjust=-8.0;
...
void printClock(void)
  ...
  // my copyright - do not remove please
  tft.setCursor(0, 303); // x,y
  tft.setTextSize(1);
...
void printTemperature(float tval)
  {
  tft.setCursor(156,246); //272
  tft.setTextSize(3);
  tft.setTextColor(ILI9341_PINK,ILI9341_BLACK);
  tft.print(tval,1);
  tft.setTextSize(2);
  tft.setCursor(228,242); // 268
 ...
 // if there are no errors, I can print clock and temperature
  if (!fatalError)
    ...
    if (tempCount==32)
      ...
      printTemperature(temp+TempAdjust);
      temp=0;
      tempCount=0;
      }
    }
